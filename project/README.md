...
- wrapper interpreter resolution: `PROJECT_INTAKE_PYTHON` -> `VIRTUAL_ENV/bin/python` -> product-local `.venv/bin/python` -> parent-repo `.venv/bin/python` (deprecated monorepo fallback, emits warning on stderr)
